...
def create_articles_by_document_type():
    """
    Create the articles by document type table from the fact and dimension tables.
    ...
    3. Calculates the total articles by document type for each day.
    4. Writes the articles by document type table to the gold layer in Delta format.
    ...
    # Step 3: Calculate Articles by Document Type
    articles_by_document_type = fact_with_all_dimensions_df.groupBy(
    ...
    articles_by_document_type_path = f"{common.get_gold_path()}articles/articles_by_document_type"
    articles_by_document_type_table_name = f"{common.get_catalog_name()}.gold.articles_by_document_type"
    articles_by_document_type.write.format("delta").mode("overwrite").option("overwriteSchema", "true").option("path", articles_by_document_type_path).saveAsTable(articles_by_document_type_table_name)

# COMMAND ----------

if __name__ == "__main__":
    create_articles_by_document_type()
