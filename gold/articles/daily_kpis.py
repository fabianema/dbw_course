...
def create_daily_kpis():
    """
    Create the daily KPIs table from the fact and dimension tables.
    ...
    3. Calculates daily KPIs such as total articles and average word count.
    4. Writes the daily KPIs table to the gold layer in Delta format.
    ...
    daily_kpis = fact_with_all_dimensions_df.groupBy(date_format("pub_date", "yyyy-MM-dd").alias("date")) \
        .agg(
    ...
    daily_kpi_path = f"{common.get_gold_path()}/articles/daily_kpis"
    daily_kpi_table_name = f"{common.get_catalog_name()}.gold.daily_kpis"
    daily_kpis.write.format("delta").mode("overwrite").option("overwriteSchema", "true").option('path', daily_kpi_path).saveAsTable(daily_kpi_table_name)
