...
def create_articles_by_source():
    """
    Create the articles by source table from the fact and dimension tables.
    ...
    3. Calculates the total articles by source for each day.
    4. Writes the articles by source table to the gold layer in Delta format.
    ...
    # Step 3: Calculate Articles by Source
    articles_by_source = fact_with_all_dimensions_df.groupBy(
    ...
    articles_by_source_path = f"{common.get_gold_path()}articles/articles_by_source"
    articles_by_source_table_name = f"{common.get_catalog_name()}.gold.articles_by_source"
    articles_by_source.write.format("delta").mode("overwrite").option("overwriteSchema", "true").option("path", articles_by_source_path).saveAsTable(articles_by_source_table_name)

# COMMAND ----------

if __name__ == "__main__":
    create_articles_by_source()
